...
async function copyContent() {
    ...
    // catch() will only run if any error is thrown by the try block
      ...
      alert("error in copying the password");
...
function calcStrength(){
    ...
    else if((hasUpper || hasNumber) && (hasLower|| hasSymbol) && passwordLength >= 4){
        setIndicator("#ff0");
    }else if(hasUpper&& passwordLength>=6){
        setIndicator("#ff0");
    }
    else if(hasLower&& passwordLength>=6){
        setIndicator("#ff0");
    }
    else{
        setIndicator("#f00");
